    ...
    @include('partials.crud-index', [
        ...
        'rows' => $media->map(function ($m) {
            ...
            if ($m->tipe === 'foto' && $m->file_path && !str_starts_with($m->file_path, 'http')) {
                ...
                $copyUrl = asset('storage/' . $m->file_path);
                $copyLabel = 'Copy Link Gambar';
                $copySuccessMessage = 'Link gambar berhasil disalin!';
            } elseif ($m->tipe === 'video') {
                $ytId = e($m->file_name);
                $preview = new \Illuminate\Support\HtmlString(
                ...
                $copyUrl = $m->file_name ? 'https://www.youtube.com/watch?v=' . $m->file_name : null;
                $copyLabel = 'Copy Link Video';
                $copySuccessMessage = 'Link video berhasil disalin!';
            } else {
                $preview = new \Illuminate\Support\HtmlString(
